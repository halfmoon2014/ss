<%@ Page Language="C#" AutoEventWireup="true" CodeFile="web_sp_cpdaxx.aspx.cs" Inherits="web_ls_web_ls_cpdaxx" %>
...
<script language="javascript" type="text/javascript">
    ...
    function ok_click() {
        if ($('#xx').tree("getSelected") != null) {//选择了一行
            ...
            var t = openModal("web_sp_cpdaxx_add.aspx?lx=dl&zt=add&id=" + id);
            if (t == "ok") {
                $('#xx').tree("reload");
            }
        } else {
    ...
    function edit_click() {
        if ($('#xx').tree("getSelected") != null) {
            ...
            //alert(mySysDate(mc))           
            if (id == "-1") {//没有大类自动生成
                $.messager.alert('提示信息', '不能修改这个大类!', 'info');
            } else {
                //alert("web_sp_cpdaxx_add.aspx?lx=dl&zt=edit&mc=" + mc + "&id=" + id);
                var t = openModal("web_sp_cpdaxx_add.aspx?lx=dl&zt=edit&mc=" + mySysDate(mc) + "&id=" + id);
    ...
    function del_click() {
        ...
        if ($('#xx').tree("getSelected") != null) {
            ...
            if ($('#xx').tree("getSelected").id == "-1") {//没有大类自动生成
                parent.$.messager.alert('提示信息', '不能删除这个大类!', 'info');
            } else if ($('#xx').tree("getSelected").attributes.xjbs == "1") {//有下级             
                parent.$.messager.alert('提示信息', '此大类有下级类别,不能删除!', 'info');
            } else {
                $('#ok').linkbutton('disable');
                //xjbs 下级标识
                var str = "";
                str = "select top 1 khlb from V_sp_cpda where khlb=" + id
                //alert(str);
                var r = myAjax(str);
                if (r == -1) {
                    ...
                    if (r.r == 'true' && r.msg == "null") {
                        //允许删除
                        //alert(r.msg);alert(1);
                        str = " delete from v_sp_xxdmb where id=" + id;
                        if (id == "") {
                            parent.$.messager.alert('提示信息', '没有可更新的记录!', 'info', function () {
                                $('#ok').linkbutton('enable');
                            });
                        } else {
                            //alert(str);
                            ...
                            if (r == -1) {
                                ...
                                if (r.r == 'true') {
                                    ...
                                    parent.$.messager.alert('提示信息', r.msg, 'info');
                                    $('#ok').linkbutton('enable');
                                }
                            }
                        }
                    } else {
                        parent.$.messager.alert('提示信息', "已经被产品档案引用,不能删除", 'info', function () {
                            $('#ok').linkbutton('enable');
                        });
                        //parent.$.messager.alert('提示信息', r.msg, 'info');                        
